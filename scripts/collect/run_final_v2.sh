#!/usr/bin/env bash
# M6 final-v2 formal collection + build + full train/eval orchestration.
#
# Runs the full overnight pipeline and is resumable: each phase records a
# .done marker under datasets/final-v2/.m6/ and is skipped on re-launch.
# Launch detached so it survives the session closing:
#     nohup scripts/collect/run_final_v2.sh > datasets/final-v2/m6_run.log 2>&1 &
#
# Phases:
#   1. normal collection  : 8 classes x 2 CVE kernels (idle, msg_msg 256/2048,
#      keyctl, net_busy, fs_io, fork_stress, mem_pressure)
#   2. attack collection  : 2 CVEs x {single_spray, combo} + no-spray baseline
#   3. build + data gates : trace2csv -> csv2features -> pilot_gates (G1-G6)
#   4. train/eval         : 8 models (base + leave-one-CVE-out x2 each)
...
# M6 model set (user decision 2026-08-21, twice revised): svm (ocsvm), lstm-ae,
# ngram + baselines mlp_ae and lstm_vae. NO leave-one-CVE-out this round -- the
# final report is a straight N-model base comparison. TOLERANT models are ones
# whose G10 is known-unstable or unknown: a completed run whose only problem is
# a FAILed gate is a valid finding and must not halt the pipeline (a real crash
# -- no "SOME M5 GATES FAILED" in the log -- still halts).
MODELS="ocsvm lstm_ae ngram mlp_ae lstm_vae"
TOLERANT_MODELS="lstm_ae ngram mlp_ae lstm_vae"
...
$PY scripts/collect/collect_attack_stable.py -c $CVES \
    ...
    --expect-crash $CVES --poc-timeout 90 -o $RAW_ATTACK
$PY scripts/collect/collect_attack_stable.py -c $CVES \
    -v poc_cfh_baseline -n $ATTACK_RUNS --expect-crash $CVES \
    --poc-timeout 90 -o $RAW_BASELINE
"

# ----------------------------------------------------------------- build ---
run_phase build_gates bash -c "
$PY scripts/validate/build_pilot_dataset.py \
    --attack-raw $RAW_ATTACK --normal-raw $RAW_NORMAL \
    --baseline-raw $RAW_BASELINE --out $DATA
"

# ---------------------------------------------------------------- train ----
# Base aggregate eval for the 5-model set (no LOO this round, per user).
# ngram uses the run-level token adapter (run_ngram.py, run-level scoring); the
# TOLERANT models above are gate-FAIL-only tolerated.
run_train() { # <name> <model>
  ...
  echo "[m6] === phase $name start $(date -Is) ==="
  local rc=0 runner extra
  if [ "$model" = "ngram" ]; then
    runner="scripts/train/run_ngram.py"
    extra=(--normal-csv-root "$DATA/csv/normal" --attack-csv-root "$DATA/csv/attack")
  else
    runner="scripts/train/run_experiment.py"
    extra=(--model "$model")
  fi
  "$PY" "$runner" "${extra[@]}" \
